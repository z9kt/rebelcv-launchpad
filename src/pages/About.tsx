import { Eye, Compass, Rocket, Lightbulb, Code, Users, Lightning } from "@phosphor-icons/react";
...
const timeline = [
  { icon: <Lightbulb size={20} weight="duotone" />, title: "Idén föds", date: "Aug 2025", description: "Frustrationen över att anpassa samma CV ledde till en fråga: varför kan inte detta automatiseras?" },
  { icon: <Code size={20} weight="duotone" />, title: "Första prototypen", date: "Sep 2025", description: "En första AI-driven matcher byggdes för att testa konceptet." },
  { icon: <Users size={20} weight="duotone" />, title: "Betatest", date: "Okt 2025", description: "Tidiga användare testade plattformen och gav ovärderlig feedback." },
  { icon: <Lightning size={20} weight="duotone" />, title: "Lansering", date: "Dec 2025", description: "RebelCV lanseras för allmänheten med full funktionalitet." },
];
          ...
          <div className="space-y-6">
            {timeline.map((item, index) => (
              <div key={index} className="flex gap-4 p-6 bg-muted/50 rounded-2xl">
                <div className="w-10 h-10 rounded-full bg-violet-600 text-white flex items-center justify-center flex-shrink-0">{item.icon}</div>
                <div>
                  <span className="text-sm text-violet-600 font-medium">{item.date}</span>
