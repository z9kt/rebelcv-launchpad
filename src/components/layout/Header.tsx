...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 backdrop-blur-xl bg-black/40 border-b border-white/5">
      <div className="container-main">
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <Link to="/" className="flex flex-col">
            ...
            <span className="text-xs text-slate-400 -mt-0.5">AI-drivet levande CV</span>
          </Link>
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed inset-0 top-16 bg-black/95 backdrop-blur-xl z-40">
          <nav className="flex flex-col p-6 space-y-6">
            ...
            <div className="pt-6 border-t border-white/10 space-y-4">
              <Link
                to="/logga-in"
                onClick={() => setMobileMenuOpen(false)}
                className="block text-xl text-slate-400"
              >
                Logga in
              </Link>
              <Link
